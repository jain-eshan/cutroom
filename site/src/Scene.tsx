 ...
 * app frames them.
 *
 * TEMPORARY EXCEPTION to the design system's "no illustrations" rule, kept at
 * the founder's call (2026-09-20) after removing it left two empty striped
 * boxes on the live page. Delete this file when real footage replaces it: see
 * site/README.md, "The hero demo". The artwork is off-system; the frames
 * around it (card, chips, timeline) use the current tokens.
 */
...
function shotLabel(shot: ShotSpec): string {
	...
	return `Close · ${PEOPLE[shot.who[0]].name}`;
}
...
/** Fixed ink for anything laid over the picture: the plate never inverts, so
 * neither can what sits on it. */
const ON_PLATE = "text-[oklch(0.92_0.005_80)]";
const PILL = `rounded-chip bg-black/55 px-2 py-[5px] font-mono text-label leading-none tracking-[0.04em] uppercase ${ON_PLATE}`;

/** Time through the 18 s loop. Starts still for anyone who asked for less motion. */
function useLoop() {
	...
	return { t, setT, playing, setPlaying, shot };
}

/** The picture itself, for whichever shot is current. */
function Frame({ shot }: { shot: ShotSpec }) {
	return (
		<>
			...
			{shot.kind === "both" && (
				<div className="flex h-full">
					{shot.who.map((id) => (
						<div key={id} className="relative h-full flex-1 border-l-2 border-plate-a first:border-l-0">
...
export function FramingDemo() {
	const { t, setT, playing, setPlaying, shot } = useLoop();
	const pct = (v: number) => `${(v / LOOP_S) * 100}%`;

	return (
		<div>
			<div className="rounded-marketing border border-line bg-raised p-[10px] shadow-hero">
				<div className="relative aspect-video overflow-hidden rounded-[8px] bg-plate-b">
					<Frame shot={shot} />

					<span className={`absolute top-3 left-3 ${PILL}`}>{shotLabel(shot)}</span>
					{/* Hidden on phones, where it would sit on top of the caption. */}
					<span className={`absolute right-3 bottom-3 hidden sm:inline ${PILL}`}>{formatTime(t)}</span>
					<p
						className={`absolute inset-x-0 bottom-[9%] mx-auto w-fit max-w-[80%] rounded-[6px] bg-black/70 px-3 py-1.5 text-center text-[clamp(11px,1.7vw,17px)] leading-snug ${ON_PLATE}`}
					>
						{shot.caption}
					</p>
				</div>
				...
				<div className="px-1 pt-3 pb-1">
					<div className="relative">
						<div className="relative h-[38px] overflow-hidden rounded-control border border-line bg-track">
							{SHOTS.map((s) => (
								<button
									type="button"
									key={s.start}
								...
								>
									...
									<span className="hidden truncate font-mono text-[9px] leading-none tracking-[0.04em] whitespace-nowrap text-r-ink uppercase sm:inline">
										{shotLabel(s)}
									</span>
								</button>
							))}
						...
						<div className="mt-1.5 flex flex-col gap-1">
							{(Object.keys(SPEECH) as PersonId[]).map((id) => (
								<div key={id} className="flex items-center gap-2">
									<span className="w-9 font-mono text-label text-text3">{PEOPLE[id].name}</span>
									<div className="relative h-[15px] flex-1 overflow-hidden rounded-chip bg-track">
										{SPEECH[id].map(([a, b]) => (
											<div key={a} className={`absolute inset-y-0 rounded-[2px] ${LANE_FILL[id]}`} style={{ left: pct(a), width: pct(b - a) }} />
										))}
									</div>
								</div>
							))}
			...
			<div className="mt-3 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-center text-[12.5px] text-text3">
				<button
					type="button"
					onClick={() => setPlaying((p) => !p)}
					className="rounded-control border border-line bg-raised px-[11px] py-[7px] text-[10.5px] leading-none font-medium text-text2 transition-colors duration-[90ms] ease-linear hover:bg-control"
				>
					{playing ? "Pause" : "Play"}
				</button>
...
/** The same edit, small: the picture alone, cutting to whoever is talking. For
 * the "It cuts to whoever's talking" block, which is a 16:10 clip. */
export function AutoFramingLoop() {
	const { shot } = useLoop();
	return (
		<div className="relative h-full w-full bg-plate-b">
			<Frame shot={shot} />
			<span className={`absolute top-2 left-2 ${PILL}`}>{shotLabel(shot)}</span>
		</div>
	);
}
